...
export function EvaluationRunner({
  ...
  const [missedSkillTags, setMissedSkillTags] = useState<Set<string>>(new Set());
  ...
  async function handleSubmit(choiceValue?: string) {
    ...
    setSubmitting(true);
    const res = await fetch(`/api/child/evaluation-question/${current.id}/submit`, {
      method: "POST",
    ...
    if (!attempted) {
      ...
      if (!data.isCorrect) {
        setMissedSkillTags((s) => new Set(s).add(current.skillTag));
      }
      setAttempted(true);
    }
  ...
  async function handleNext() {
    ...
    if (index + 1 >= questions.length) {
      ...
      await fetch(`/api/child/evaluation/${evaluationId}/complete`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          correctAnswers: correctCount,
          totalQuestions: questions.length,
          missedSkillTags: Array.from(missedSkillTags),
        }),
      });
      setDone(true);
    } else {
  ...
  if (questions.length === 0) {
    return (
      <p className="text-sm text-slate-500">
        Cette évaluation n&apos;a pas encore de questions.
      </p>
    );
  }
  ...
  if (done) {
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center">
        ...
        <p className="font-semibold text-slate-900 mb-1">Évaluation terminée !</p>
